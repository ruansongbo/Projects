...
namespace CommandDalayTestServer
{
    public partial class Server : Form
    {
        ...
        private void aTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            ...
            {
                //将需要发送的字符串转换为Byte数组
                Byte[] sendByte = Encoding.Default.GetBytes(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff"));
                //向客户端异步发送消息
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            serverIPEndPoint = new IPEndPoint(Dns.GetHostEntry(Dns.GetHostName()).AddressList.FirstOrDefault<IPAddress>(a => a.AddressFamily.ToString().Equals("InterNetwork")), listenPort);//实例化地址端口类
            //实例化一个TCP流式套接字
